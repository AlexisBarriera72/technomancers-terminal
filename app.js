...
(function () {
  ...
  function spend() {
    ...
    C.gear.forEach(function (g) { t += parseCredits(g.cost); });
    return t;
  }
  function mySub() {
  ...
  function quickBuild() {
    ...
    if (fl) {
      ...
      if (gun) C.gear.push({ key: "Firearm List|" + gun[0], name: gun[0], cost: gun[1] });
    }
    if (ar) {
      var arm = ar.rows.filter(function (r) { return /steelcloth/i.test(r[0]); })[0];
      if (arm) C.gear.push({ key: "Armor|" + arm[0], name: arm[0], cost: arm[1] });
    }
    ...
    ["Firearm List", "Armor", "Melee Weapons", "Gear List"].forEach(function (tn) {
      ...
      rows.forEach(function (r) {
        ...
        b.onclick = function () {
          ...
          else C.gear.push({ key: key, name: r[0], cost: r[1] });
          delete C.isExample; save(); render();
        };
        cell.appendChild(b);
